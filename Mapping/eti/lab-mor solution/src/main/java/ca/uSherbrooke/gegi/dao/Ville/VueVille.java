...
public class VueVille {
    ...
    public static int getIDVille(String libelle) {
        Injector injector = Guice.createInjector(new Module());
        VilleMapper villeMapper = injector.getInstance(VilleMapper.class);
        return villeMapper.selectVilleID(libelle);
    }

    public static String getNomVille(int id) {
        Injector injector = Guice.createInjector(new Module());
        VilleMapper villeMapper = injector.getInstance(VilleMapper.class);
        return villeMapper.selectCity(id);
    }
}
